...
export class GitWorktreeManager {
  ...
  create(intake: RepositoryIntake, runId: string, now = new Date().toISOString()): WorktreeRecord {
    ...
    const worktreesRoot = join(this.runtimeRoot, "worktrees");
    const recordsRoot = join(this.runtimeRoot, "worktree-records");
    ...
    writeFileSync(markerPath, `${JSON.stringify(intent)}\n`, { encoding: "utf8", mode: 0o600, flag: "wx" });
    ...
    const completedMarker = `${markerPath}.complete`;
    writeFileSync(completedMarker, `${JSON.stringify(record)}\n`, { encoding: "utf8", mode: 0o600, flag: "wx" });
    renameSync(completedMarker, markerPath);
    return record;
  }
  ...
  discover(runId: string): WorktreeRecord | undefined {
    ...
    if (actual !== record.path || !actual.startsWith(`${join(this.runtimeRoot, "worktrees")}/`)) {
  ...
  private assertRuntimeOutsideRepository(repositoryPath: string): void {
    const rel = relative(repositoryPath, this.runtimeRoot);
    if (rel === "" || (!rel.startsWith("..") && !isAbsolute(rel))) {
      throw new KerbsFlowError("RUNTIME_ROOT_UNSAFE", "runtime root must be outside the human-owned repository");
    }
  }
}
...
function renderUntrackedDiff(worktreePath: string, status: GitStatusEntry[], remainingBytes: number): Buffer {
  ...
  for (const entry of status) {
    ...
    if (!candidate.startsWith(`${worktreePath}/`)) {
...
function git(cwd: string, args: string[]): string {
  try {
    return execFileSync("git", args, { cwd, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], maxBuffer: 4 * 1024 * 1024 }).trim();
  } catch (error) {
...
function gitOptional(cwd: string, args: string[]): string {
  try {
    return execFileSync("git", args, { cwd, encoding: "utf8", stdio: ["ignore", "pipe", "ignore"], maxBuffer: 1024 * 1024 }).trim();
  } catch {
    return "";
  }
}
...
function gitBuffer(cwd: string, args: string[]): Buffer {
  try {
    return execFileSync("git", args, { cwd, encoding: "buffer", stdio: ["ignore", "pipe", "pipe"], maxBuffer: 4 * 1024 * 1024 });
  } catch (error) {
    const detail = error instanceof Error ? error.message : "Git command failed";
    throw new KerbsFlowError("GIT_COMMAND_FAILED", `git ${args[0] ?? "command"} failed: ${detail}`);
  }
